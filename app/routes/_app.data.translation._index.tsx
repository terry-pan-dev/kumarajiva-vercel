...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (intent === 'create-section') {
    ...
    await createSectionTitle({ sectionId, documentId, title: originTitle }, user);

    if (targetDocumentId && translationTitle) {
      await createSectionTitle({ sectionId, documentId: targetDocumentId, title: translationTitle }, user);
    }
  ...
  if (intent === 'update-section') {
    const sectionId = formData.get('sectionId') as string;
    const targetDocumentId = (formData.get('targetDocumentId') as string) || null;
    const originDocumentId = formData.get('documentId') as string;
    const originTitle = formData.get('originTitle') as string;
    const translationTitle = (formData.get('translationTitle') as string) || '';

    await updateSectionTitle({ sectionId, documentId: originDocumentId, title: originTitle }, user);

    if (translationTitle && targetDocumentId) {
      await updateSectionTitle({ sectionId, documentId: targetDocumentId, title: translationTitle }, user);
    }
