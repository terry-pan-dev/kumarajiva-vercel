...
import type { DocumentSection } from '~/services/text.service';
...
export type ProjectForRow = {
  id: string;
  name: string;
  sourceDocument: DocumentForRow;
  targetDocument: DocumentForRow;
};
      ...
      {isOpen && (
        <>
          ...
          >
            {sections.length > 0 ? (
              (() => {
                const targetTitleById = new Map(project.targetDocument.sections.map((s) => [s.id, s.title]));
                return sections.map((section) => (
                  <DraggableSectionRow
                    ...
                    targetTitle={targetTitleById.get(section.id) ?? null}
                  />
                ));
              })()
            ) : (
